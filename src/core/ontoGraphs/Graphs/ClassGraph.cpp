...
void ClassGraph::add(const std::string& value, ObjectVectors_t& object_vector)
{
  ...
  **********************/
  for(size_t property_i = 0; property_i < object_vector.object_relations_.size(); property_i++)
    addObjectProperty(me, object_vector.object_relations_[property_i].first, object_vector.object_relations_[property_i].second, object_vector.object_relations_[property_i].probability);
...
void ClassGraph::addObjectProperty(ClassBranch_t* me, std::string& property, std::string& class_on, float probability)
{
  ObjectPropertyBranch_t* property_branch = nullptr;
  getInMap(&property_branch, property, object_property_graph_->roots_);
  getInMap(&property_branch, property, object_property_graph_->branchs_);
  getInMap(&property_branch, property, object_property_graph_->tmp_mothers_);
  if(property_branch == nullptr)
  {
    ObjectPropertyVectors_t empty_vectors;
    object_property_graph_->add(property, empty_vectors);
    getInMap(&property_branch, property, object_property_graph_->roots_);
  }

  ClassBranch_t* class_branch = nullptr;
  getInMap(&class_branch, class_on, roots_);
  getInMap(&class_branch, class_on, branchs_);
  getInMap(&class_branch, class_on, tmp_mothers_);
  if(class_branch == nullptr)
  {
    class_branch = new ClassBranch_t(class_on);
    tmp_mothers_[class_branch->value()] = class_branch;
  }

  if(probability != 1.0)
    me->object_relations_.push_back(ObjectRelationElement_t(property_branch, class_branch, probability));
  else
    me->setSteady_objectRelation(ObjectRelationElement_t(property_branch, class_branch));
}
...
void ClassGraph::setSteadyObjectProperty(ClassBranch_t* branch_from, ObjectPropertyBranch_t* branch_prop, ClassBranch_t* branch_on)
{
  ...
  if(found == false)
    branch_from->steady_.object_relations_.push_back(ObjectRelationElement_t(branch_prop, branch_on));
  ...
  if(found == false)
    branch_from->object_relations_.push_back(ObjectRelationElement_t(branch_prop, branch_on));
}
