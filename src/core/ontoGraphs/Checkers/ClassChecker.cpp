...
void ClassChecker::checkObectPropertyDomain()
{
  for(size_t i = 0; i < graph_size; i++)
  {
    ...
    for(size_t prop_i = 0; prop_i < graph_vect_[i]->object_relations_.size(); prop_i++)
    {
      ...
      class_graph_->object_property_graph_->getUpPtr(graph_vect_[i]->object_relations_[prop_i].first, prop_up);
...
void ClassChecker::checkObectPropertyRange()
{
  for(size_t i = 0; i < graph_size; i++)
  {
    ...
    {
      std::unordered_set<ClassBranch_t*> up;
      class_graph_->getUpPtr(graph_vect_[i]->object_relations_[prop_i].second, up);
...
void ClassChecker::checkDataPropertyDomain()
{
  for(size_t i = 0; i < graph_size; i++)
  {
    ...
    for(size_t prop_i = 0; prop_i < graph_vect_[i]->data_relations_.size(); prop_i++)
    {
      ...
      class_graph_->data_property_graph_->getUpPtr(graph_vect_[i]->data_relations_[prop_i].first, prop_up);
      std::unordered_set<ClassBranch_t*> domain;
      for(auto prop : prop_up)
...
void ClassChecker::checkDataPropertyRange()
{
  ...
  for(size_t i = 0; i < graph_size; i++)
  {
    for(size_t prop_i = 0; prop_i < graph_vect_[i]->data_relations_.size(); prop_i++)
    {
      std::unordered_set<std::string> range = class_graph_->data_property_graph_->getRange(graph_vect_[i]->data_relations_[prop_i].first->value());
      if(range.size() != 0)
      {
