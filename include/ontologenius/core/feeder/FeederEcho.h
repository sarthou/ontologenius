...
public:
  ...
  void add(const std::string& data, const compat::ros::Time& stamp = compat::ros::Node::get().current_time())
  {
    mut_.lock();
  ...
  void add(const std::vector<std::pair<std::string, RosTime_t>>& facts)
  {
    mut_.lock();
    for(auto& fact : facts)
      echo_messages.emplace_back(fact.first, compat::ros::Time { (uint16_t) fact.second.sec, fact.second.nsec } );
    mut_.unlock();
  }
...
private:
  std::mutex mut_;
  compat::ros::Publisher<compat::OntologeniusStampedString> feeder_echo_pub_;
  compat::ros::Publisher<compat::OntologeniusExplanation> feeder_explanation_pub_;

  std::vector<std::pair<std::string, compat::ros::Time>> echo_messages;
