...
namespace onto {
  ...
  public:
    ...
    /// @param index is the concept indentifier for which you search a label.
    ...
    /// @param index is the concept indentifier for which you search its labels.
    /// @param take_id can be set to false if you do not want to consider the concept identifier as a possible default name.
    /// @return The result of this function depends on the setting of the working language.
    ...
    std::vector<std::string> getComments(int64_t index);
    /// @brief Gives the comments associated to the concept referenced by the provided index, if it exists
    /// (i.e. class, individual, object property, data property).
    /// @param index is the concept indentifier for which you search its comments.
